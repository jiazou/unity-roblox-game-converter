...
| Unity role | Roblox module | Bridge modules |
|---|---|---|
| Central manager + state subclasses | State modules + bootstrap wiring via `StateMachine` | `StateMachine` |
| World/level manager | Dedicated manager module | `GameObjectUtil`, `Time` |
| Input/character controller | Dedicated controller module | `Input`, `Physics` |
| Game-specific MonoBehaviours | One module per behaviour | `MonoBehaviour` |
| Legacy Animation on non-skeletal objects | Auto-generated `*_TransformAnimConfig` | `TransformAnimator` |
| Mecanim Animator on skinned meshes | Auto-generated `*_RootMotionConfig` | `TransformAnimator` |
...
- **No `_meta.json` needed for script types.** The converter auto-detects: files ending with `return <identifier>` → ModuleScript; files using client APIs (`Players.LocalPlayer`, `UserInputService`) → LocalScript; otherwise Script. Override via `_meta.json` only when auto-detection is wrong.
...
- Registers states with the StateMachine bridge; starts the state machine with the initial state.
...
Unity scenes often have both a player *prefab* (character model with camera, slot children, etc.) AND a *spawn marker* part sharing a similar or identical name. The converter places both in Workspace. When resolving the spawn position:
...
- **Editor preview instances** (prefabs placed in the scene for editor viewing but spawned at runtime): the converter now auto-excludes these (they have `source_prefab_name` set). Any that slip through (non-prefab copies) must be hidden manually.
