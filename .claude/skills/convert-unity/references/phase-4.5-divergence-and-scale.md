...
Unity uses 1 unit ≈ 1 meter. Roblox avatars are ~5.5 studs tall vs Unity's ~1.8 units. The converter passes positions 1:1, so a Roblox avatar is ~4× larger than a typical Unity character.
...
- **Scale world up.** Multiply positions/sizes by a uniform factor. Simpler, but re-run the converter and watch for broken mesh proportions.
...
**World-space computation.** Unity stores transforms as local-space. The converter computes world-space recursively: `world_pos = parent_pos + parent_rot * local_pos`; `world_scale = parent_scale × node_scale`. If objects cluster at the origin, check `conversion_helpers.py:_compute_world_transform()`.

**FBX bounding box sizing.** MeshPart sizes are derived from FBX bounds in `_get_fbx_bounds()`, scaled by the FBX `UnitScaleFactor` and Unity's `.fbx.meta` settings (`globalScale`, `useFileScale`). Three things must be right:
...
**Decoration positions are baked into prefabs — preserve them faithfully.** Artists pre-position all decoration children at specific local offsets. There is no runtime repositioning at the decoration level. Never override or "fix" these positions in the converter output. If decorations appear to block the play area, the root cause is elsewhere (camera angle, character scale, mesh orientation) — not the baked positions.

## Mesh facing direction

The converter passes positions and rotations 1:1. FBX meshes are uploaded via assimp (FBX→GLB) with no coordinate system transform. Unity is left-handed Y-up (Z-forward); Roblox and glTF are right-handed Y-up. Mesh geometry baked into the FBX may face the wrong direction.
