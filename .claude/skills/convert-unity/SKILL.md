...
4. **Platform divergence analysis** — This is the most critical step. Unity is a blank canvas: no default character, camera, input, or physics. You build everything from scratch. Roblox is the opposite: every player spawns with a fully working character, camera, input system, and physics body. The port must decide, for each of these four pillars, whether the Unity game's approach matches Roblox's defaults or conflicts with them.
   ...
   | **Asset loading** | Unity loads meshes/textures from the Asset pipeline — once imported, they're embedded in the build and available immediately via Renderer/MeshFilter references. `Addressables.LoadAssetAsync` handles dynamic loading. | `MeshId` is **read-only at runtime** — scripts cannot set it. Meshes must be loaded via `InsertService:LoadAsset(assetId)` which returns a Model. To use the mesh, **clone** the MeshPart from the loaded Model and place the clone in the scene. You cannot create an empty MeshPart and assign its mesh later. | **Always for uploaded FBX assets.** The pipeline uploads FBX files as Model assets, then generates a MeshLoader script that loads them via InsertService at runtime. Scene MeshParts are placeholders that get **replaced** (not patched) by clones of the loaded assets. **The game bootstrap MUST wait for MeshLoader to finish** (`ReplicatedStorage:WaitForChild("MeshLoaderDone", 120)`) **before entering gameplay.** MeshLoader takes 20-30s to load ~175 assets; if the game starts spawning content before templates are replaced, all cloned objects will have placeholder geometry (gray boxes) instead of real meshes. **Skinned meshes** (FBX files with bone/skeleton data, from SkinnedMeshRenderer components) are invisible as static MeshParts because Roblox tries to apply nonexistent bone transforms. The pipeline strips skinning data (`skins`, `JOINTS_`, `WEIGHTS_`) during FBX→GLB conversion, leaving valid bind-pose static geometry. If a mesh is invisible despite correct MeshId/Size/Transparency, check `assimp info <file>.fbx` for `Bones: N` — any value > 0 means skinning data that must be stripped. See `references/upload-patching.md` for the full pattern. |
   | **Object visibility** | In Unity, only GameObjects with a MeshRenderer, SkinnedMeshRenderer, or SpriteRenderer are visible. Everything else — script containers, empty transforms, trigger volumes, collider-only objects, disabled GameObjects, disabled renderers — is invisible by design. A Unity scene typically has far more invisible objects than visible ones. | Every Part is visible by default. There is no concept of a "renderer component" separate from the object. A Part with no mesh and no special properties still renders as a gray block. | **Always.** The pipeline sets `Transparency=1` on any object without a renderer component. This is the single most important visibility rule: **no renderer = invisible**. Only `MeshRenderer` and `SkinnedMeshRenderer` count as 3D renderers — `SpriteRenderer` objects (graffiti decals, shadow circles, glow effects) are 2D overlays with no mesh geometry and must also be hidden. Unity built-in Quad (fileID 10210) and Plane (fileID 10209) primitives are almost always flat transparent effect surfaces (shadows, billboards) and are hidden automatically. Additionally: trigger colliders (`isTrigger`), inactive GameObjects (`m_IsActive=0`), disabled renderers (`m_Enabled=0`), and UI subtrees (Canvas hierarchies) are all handled. If opaque gray/yellow rectangles block the view in the converted game, check: (1) SpriteRenderer nodes not hidden, (2) Quad/Plane effect surfaces not hidden, (3) MeshLoader race condition (see Asset loading row). |
...
7. **Transform animation detection** — Two distinct animation paths exist:
   - **Legacy Animation** (classID 111): `.anim` files driving looping transform animations (spin, bob, tilt) on non-skeletal objects like collectibles and power-ups. The converter's `convert_transform_animations()` auto-generates TransformAnimator configs from parsed `.anim` keyframe data.
   - **Mecanim Animator on skinned meshes**: When the pipeline strips skinning data from FBX files (to make SkinnedMeshRenderer objects render as static MeshParts), it also searches for associated animation FBXes (`<Name>_Run.fbx`, `_Walk.fbx`, `_Idle.fbx`, `_Anim.fbx`). If found, `extract_fbx_root_motion()` reads the Hips bone's translation/rotation keyframes and generates a `*_RootMotionConfig` ModuleScript. MeshLoader sets `HasRootMotion`/`RootMotionConfig` attributes on the template; spawning code should check these attributes and create a `TransformAnimator` per clone. The result is a looping bob/sway — not full skeletal animation, but convincing for small fast-moving obstacles.
